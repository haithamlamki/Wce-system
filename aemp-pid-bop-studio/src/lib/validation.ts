...
/** Do two component boxes overlap (optionally shrunk by a clearance margin)? */
function overlaps(a: Component, b: Component, gap: number): boolean {
  const ba = box(a);
  const bb = box(b);
  return (
    a.x - gap < b.x + bb.w &&
...
export function validate(project: Project, opts: ValidateOptions = {}): Issue[] {
  ...
  // 1. Overlap / clearance — pairwise box intersection
  for (let i = 0; i < live.length; i++) {
    for (let j = i + 1; j < live.length; j++) {
      const a = live[i];
      const b = live[j];
      if (a.groupId && a.groupId === b.groupId) continue; // grouped items may abut
      if (overlaps(a, b, clearance)) {
        const touching = overlaps(a, b, 0);
        issues.push({
          id: `ov-${a.id}-${b.id}`,
          kind: touching ? 'overlap' : 'clearance',
          severity: touching ? 'warn' : 'warn',
          message: touching
            ? `${a.tag || SYM[a.type as SymbolKey].name} overlaps ${b.tag || SYM[b.type as SymbolKey].name}`
            : `${a.tag || SYM[a.type as SymbolKey].name} is closer than ${clearance}px to ${b.tag || SYM[b.type as SymbolKey].name}`,
          nodeIds: [a.id, b.id],
          at: midpoint(a, b),
        });
      }
